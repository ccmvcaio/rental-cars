...
feature 'Admin searches rental' do
  ...
  scenario 'and find by partial token' do
    user = User.create!(name: 'Caio Valério', email: 'caio.valerio@gmail.com',
                        password: '123456')
    car_category = CarCategory.create!(name: 'Econo', daily_rate: 105.5,
                                       car_insurance: 58.5, third_party_insurance: 10.5)
    ...
    second_rental = Rental.create!(start_date: Date.current, end_date: 1.day.from_now,
                                   car_category: car_category, user: user, 
                                   client: client)
    second_rental.update(token: 'ABC222')
    third_rental = Rental.create!(start_date: Date.current, end_date: 1.day.from_now,
                                   car_category: car_category, user: user, 
                                   client: client)
    ...
    visit root_path
    click_on 'Locações'
    fill_in 'Buscar locação', with: 'ABC'
    click_on 'Buscar'

    expect(page).to have_content(rental.token)
    expect(page).to have_content(second_rental.token)
    expect(page).to_not have_content(third_rental)
  end
end